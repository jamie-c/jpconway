...
const NavLinksHorizontal = () => {
    ...
    return (
        <span>
            <nav className="flex-10 flex flex-row justify-end gap-4 text-xl text-tricornBlack-500">
...
export const NavLinksVertical = () => {
    ...
    return (
        <span>
            <nav className="flex flex-col justify-start items-end gap-8 text-5xl text-tricornBlack-500 pt-10 pr-8">
                {mainNavItems.map(([ title, url ]) => (
                    <NavItem key={title} title={title} url={url} />
                ))}
            </nav>
